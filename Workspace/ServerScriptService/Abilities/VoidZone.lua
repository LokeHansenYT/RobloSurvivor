-- Abilities/VoidZone.lua
-- Ground VOID: gør fjender 'Vulnerable' (+flat dmg) 1s mens de står i feltet; kill i feltet udvider + nulstiller tid.
...
local BASE_TTL             = 4
local MIN_DIST, MAX_DIST   = 12, 36 -- ~2–6 “plader”
local BASE_SIZE            = Vector3.new(14, 0.2, 14)
local MAX_SCALE            = 2.0
local EXPAND_ON_KILL       = 0.10

-- roterende gevinster pr. level: +1s varighed ? +1 flat dmg ? +10% størrelse ? gentag
...
function M.run(player, level, C)
	...
	p.Transparency = 1 -- sprite-mode; fallback neon sættes i SpriteFX hvis sheet mangler
	p.Anchored = true
	p.CanCollide = false
	p.Size = BASE_SIZE
	...
	-- spritesheet (8×8 som standard; fallback til hvad du har sat i SpriteFX)
	SpriteFX.addFlipbookOrGui(p, {
		-- sæt Workspace attribute "VoidTwirlsId" til dit uploadede sheet
		image = SpriteFX.Assets and (SpriteFX.Assets.VoidTwirls or SpriteFX.Assets.Embers) or nil,
	...
	local field = FieldCore.new(p, {
		...
		ttl            = ttl,
		maxScale       = MAX_SCALE * (1 + extraScale),
		expandPctOnKill= EXPAND_ON_KILL,
		onEnemyTick    = function(model, hum)
			Common.applyVulnerable(model, addDmg, 1.0)
		end,
...
function M.start(player)
	local up = player:WaitForChild("Upgrades"):WaitForChild(Ab.VoidZone.levelKey)
	while player.Parent do
		local lvl = up.Value
		if lvl > 0 and not Common.offensivePaused(player) then
			M.run(player, lvl, Common)
		end
		Common.waitScaled(player, 2.8)
	end
end
